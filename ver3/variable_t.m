...
%% 성능 평가 변수 설정
true_positive_Alice = zeros(1, length(T_values)); % Alice 탐지 성공
false_positive_Alice = zeros(1, length(T_values)); % Alice 탐지 실패 (Eve로 탐지)

true_positive_Eve = zeros(1, length(T_values)); % Eve 탐지 성공
false_positive_Eve = zeros(1, length(T_values)); % Eve 탐지 실패 (Alice로 탐지)

% A->B AR 요청
for i = 1:1:length(T_values)
     T = T_values(i);
...
%plot(T_values, true_positive_Alice, '-bo', 'DisplayName', 'SNR=5dB, Alice 탐지 성공 (True Positive)');
plot(T_values, true_positive_Alice, '-rx', 'DisplayName', 'SNR=10dB, Alice 탐지 성공 (True Positive)');
%plot(T_values, false_positive_Alice, '-rx', 'DisplayName', 'Alice 탐지 실패 (False Negative)');

% Eve에 대한 탐지 확률 및 오탐지 확률
%plot(T_values, true_positive_Eve, '-g^', 'DisplayName', 'SNR=5dB, Eve 탐지 성공 (True Positive)');
plot(T_values, true_positive_Eve, '-ms', 'DisplayName', 'SNR=10dB, Eve 탐지 성공 (True Positive)');
%plot(T_values, false_positive_Eve, '-ms', 'DisplayName', 'Eve 탐지 실패 (False Negative)');

xlabel("T");
ylabel('Probability');
legend show;
title('임계값 T에 따른 탐지 확률');
grid on;
