...
% N = 256; % 각 노드에서 생성한 랜덤한 값의 길이
N_values = 4:4:512;

nSymbols = 100000; %전체 비트 길이
...
%% 실험 과정 1 (생략 가능)
% A->B AR 요청
for i = 1:1:length(N_values)
    N = N_values(i); % 현재 N 값 설정
    nSymbol = nSymbols-N; %생성된 기존 비트 데이터
    ...
    true_probability(i) = sum(key_a == recovered_key) / length(key_a); % 정상 탐지 확률
    % key_errors = sum(key_a ~= recovered_key);
    % disp(['정상 키 에러: ', num2str(key_errors)]);
    ...
    % 오탐지 확률 계산
    false_probability(i) = sum(key_a == recovered_key) / length(key_a); % 오탐지 확률
    
    % key_errors = sum(key_a ~= recovered_key);
    % disp(['도청 키 에러: ', num2str(key_errors)]);
end

%% 탐지 및 오탐지 확률 그래프 그리기
figure;
plot(N_values, true_probability, '-o', 'DisplayName', '탐지 확률 (P_d)');
hold on;
plot(N_values, false_probability, '-x', 'DisplayName', '오탐지 확률 (P_f)');
xlabel('N 값 (랜덤 키 길이)');
ylabel('Probability');
legend show;
title('N 값에 따른 탐지 및 오탐지 확률');
grid on;
