...
%% 성능 평가 변수 설정
true_positive_Alice = zeros(1, length(alpha_values)); % Alice 탐지 성공
false_positive_Alice = zeros(1, length(alpha_values)); % Alice 탐지 실패 (Eve로 탐지)

true_positive_Eve = zeros(1, length(alpha_values)); % Eve 탐지 성공
false_positive_Eve = zeros(1, length(alpha_values)); % Eve 탐지 실패 (Alice로 탐지)
...
%plot(alpha_values, true_positive_Alice, '-bo', 'DisplayName', 'T=0.8, Alice 탐지 성공 (True Positive)');
plot(alpha_values, true_positive_Alice, '-rx', 'DisplayName', 'T=0.9, Alice 탐지 성공 (True Positive)');
%plot(alpha_values, false_positive_Alice, '-rx', 'DisplayName', 'Alice 탐지 실패 (False Negative)');

% Eve에 대한 탐지 확률 및 오탐지 확률
%plot(alpha_values, true_positive_Eve, '-g^', 'DisplayName', 'T=0.8, Eve 탐지 성공 (True Positive)');
plot(alpha_values, true_positive_Eve, '-ms', 'DisplayName', 'T=0.9, Eve 탐지 성공 (True Positive)');
%plot(alpha_values, false_positive_Eve, '-ms', 'DisplayName', 'Eve 탐지 실패 (False Negative)');

xlabel('상대적 신호 강도');
ylabel('Probability');
legend show;
title('수신 크기에 따른 탐지 확률');
grid on;
